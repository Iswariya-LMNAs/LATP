...
class clActionExpandSection extends clAction {
  ...
  executeAction(): void {
    ...
    if (this.actionRow.tab) {
      const LOtabClick = clActionFactory.createAction("On Tab", [
        this.actionRow,
      ]);
      LOtabClick.executeAction();
    }
    ...
    cy.get(".section-head").each(($el) => {
      ...
      if (Ltext === LsectionTitle) {
        ...
        const LisCollapsed =
          $parent.find(".section-body").css("display") === "none";
        if (LisCollapsed) {
...
class clActionOnLoad extends clAction {
  ...
  checkFieldValue(): void {
    super.checkFieldValue();
  }
  checkFieldProperties(): void {
    super.checkFieldProperties();
  }
  constructor(iAction: string, iaActionData: TTactionsData) {
    super(iAction, iaActionData);
  }
...
class clActionOnChange extends clAction {
  executeAction(): void {
    ...
    if (this.actionRow.is_child) {
      const LOchildAction = new clActionOnChangeChild(
        this.action,
        this.actionData
      );
...
class clActionAddRow extends clAction {
  executeAction(): void {
    ...
    cy.get(`[data-fieldname="${this.actionRow.child_name}"]`, {
      timeout: 10000,
    })
      .should("exist")
      .should("be.visible")
...
class clActionSave extends clAction {
  executeAction(): void {
    cy.get("body").then(($body: JQuery<HTMLElement>) => {
      ...
      if ($saveBtn.length > 0) {
        cy.wrap($saveBtn).scrollIntoView().click({ force: true });
        cy.log(`saved successfully.`);
        cy.wait(fnGetDelay("short"));
      } else {
        throw new Error(
          "No visible Save button (.primary-action) found in DOM."
        );
      }
    });
  }
...
class clActionDelete extends clAction {
  executeAction(): void {
    ...
    cy.contains("a.dropdown-item", "Delete")
      .should("be.visible")
      .click({ force: true });
    cy.wait(fnGetDelay("medium"));
    cy.get(".modal:visible").within(() => {
...
class clActionActionMenu extends clAction {
  executeAction(): void {
    cy.log("action row", JSON.stringify(this.actionRow));
    this.actionRow = this.actionData[0];
...
export class clActionFactory {
  ...
  static createAction(
    ...
    return new LAactionClass(
      (iAction = iAction),
      (iaActionData = iaActionData)
    );
  }
  static filterActionData(
    iaActionsData: TTactionsData,
    iActionRow: TactionData
  ): TTactionsData {
    const LposNext = iActionRow.pos + 10;
    return iaActionsData.filter(
      (ldItem) => ldItem.pos >= iActionRow.pos && ldItem.pos < LposNext
    );
  }
  static executeAction(data: TtestHeaderData[]): void {
    ...
    LvalidRows.forEach((row) => {
      const Ldoctype = row.doctype_to_be_tested
        .trim()
        .toLowerCase()
        .replace(/\s+/g, "-");
  ...
  static handleConnection(
    ...
    if (connection === "Create") {
      ...
      return cy
        .contains(".nav-item", "Connections", { timeout: 10000 })
        .should("be.visible")
        .click()
        ...
        .then(() => {
          return cy.get(".form-dashboard", { timeout: 10000 }).within(() => {
            return cy
              .contains(".document-link-badge", connection_doctype, {
                timeout: 10000,
              })
              .should("be.visible")
